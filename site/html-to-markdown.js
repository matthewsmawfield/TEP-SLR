...
/**
 * HTML to Markdown Converter for TEP-SLR Site
...
class HTMLToMarkdownConverter {
    ...
    htmlToMarkdown(html) {
        ...
        html = html.replace(/<figure[^>]*>(.*?)<\/figure>/gis, (match, content) => {
            const imgMatch = content.match(/<img[^>]+src=["']([^"']+)["'][^>]+alt=["']([^"']*)["'][^>]*>/i);
            const captionMatch = content.match(/<figcaption[^>]*>(.*?)<\/figcaption>/is);
            
            if (imgMatch) {
                const src = imgMatch[1];
                const alt = imgMatch[2];
                // Clean up caption tags but keep bolding for "Figure X:"
                let caption = '';
                if (captionMatch) {
                ...
                // Adjust path for markdown: html uses "figures/", repo has "results/figures/"
                // Markdown is in root, so we need results/figures/
                const mdSrc = src.replace(/^figures\//, 'results/figures/');
        ...
        // Remove leading whitespace from lines to fix indentation issues
