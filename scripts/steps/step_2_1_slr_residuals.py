...
def collect_np2_files(root: Path, start: date, end: date, max_files: int = 0) -> List[Path]:
    files_all = sorted(root.rglob("*.np2"))
    selected: List[Path] = []
    for p in files_all:
...
def iter_crd_observations(np2_path: Path) -> Iterable[CRDObs]:
    ...
    with open(np2_path, "r", encoding="utf-8", errors="replace") as f:
        for line in f:
            ...
            sat = current_satellite or "unknown"
...
def select_sp3_file_for_satellite(session: requests.Session, sat: str, start_date: datetime, end_date: datetime, cache_dir: Path) -> Optional[Path]:
    ...
    Returns path to SP3 file, or None if not found.
    """
    ...
    if sat_cache.exists():
        for week_dir in sorted(sat_cache.iterdir(), reverse=True):
            ...
            try:
                ...
                # Accept file if it overlaps with observation period (not requiring full coverage)
                if file_end >= start_date and file_start <= end_date:
                    logger.info(f"SP3 orbit found (cached): {chosen.name} (covers {file_start.date()} to {file_end.date()})")
                    return chosen
            except Exception:
                continue
...
def main() -> int:
    ...
    parser.add_argument("--satellites", default="lageos1,lageos2",
                        help="Comma-separated list of satellites to process (default: lageos1,lageos2). Use 'all' to try all satellites.")
    ...
    np2_root = DATA_ROOT / "npt_crd_v2" / "allsat"
    if not np2_root.exists():
        logger.error(f"Missing NP2 directory: {np2_root}")
        return 2

    np2_files = collect_np2_files(np2_root, start, end, max_files=args.max_np2_files)
    ...
    obs_start = min(df_obs["epoch_utc"])
    obs_end = max(df_obs["epoch_utc"])
    ...
    for r in df_obs.itertuples(index=False):
        ...
        # Compute tropospheric delay (to be subtracted from observed range)
        tropo_delay_m = compute_tropospheric_delay_m(elev_deg, sta_lat, sta_h)
        
        # Get center-of-mass correction for this satellite
        com_correction_m = LAGEOS_COM_CORRECTION_M.get(sat.lower(), 0.0)
        
        # Compute Shapiro relativistic delay (two-way)
        shapiro_delay_m = 2.0 * compute_shapiro_delay_m(sta_ecef, sat_ecef)
        
        # Corrected observed range: subtract tropo delay, subtract Shapiro, add CoM offset
        # (CoM is added because obs range is to retroreflector, model is to CoM)
        # (Shapiro is subtracted because it's extra delay in the observed TOF)
        obs_range_corrected_m = float(r.obs_range_m) - tropo_delay_m - shapiro_delay_m + com_correction_m
        ...
        rows.append(
            {
                ...
                "shapiro_delay_m": shapiro_delay_m,
                "residual_m": resid_m,
    ...
    summary = {
        ...
        "residual_mm": {
            "mean": float(df["residual_mm"].mean()),
            "std": float(df["residual_mm"].std(ddof=1)) if df.shape[0] > 1 else 0.0,
            "p05": float(df["residual_mm"].quantile(0.05)),
            "p50": float(df["residual_mm"].quantile(0.50)),
            "p95": float(df["residual_mm"].quantile(0.95)),
        },
        "by_station_rms_mm": (
            df.groupby("station")["residual_mm"].apply(lambda s: float(np.sqrt(np.mean(np.square(s.values))))).sort_values(ascending=False).head(25).to_dict()
        ),
        "by_satellite_rms_mm": (
            df.groupby("satellite")["residual_mm"].apply(lambda s: float(np.sqrt(np.mean(np.square(s.values))))).sort_values(ascending=False).head(25).to_dict()
        ),
    }
    ...
    try:
        ...
        plt.hist(df["residual_mm"].values, bins=120)
        plt.xlabel("Residual (mm) = observed range − CPF modeled geometric range")
        plt.ylabel("Count")
        plt.title("SLR residual distribution")
        plt.tight_layout()
        plt.savefig(FIGURES_DIR / "step_2_1_slr_residual_hist.png", dpi=160)
        plt.close()

        plt.figure(figsize=(8, 4))
        plt.scatter(df["elevation_deg"].values, df["residual_mm"].values, s=2, alpha=0.25)
        plt.xlabel("Elevation (deg)")
        plt.ylabel("Residual (mm)")
        plt.title("SLR residual vs elevation")
        plt.tight_layout()
        plt.savefig(FIGURES_DIR / "step_2_1_slr_residual_vs_elevation.png", dpi=160)
        plt.close()
    except Exception as e:
