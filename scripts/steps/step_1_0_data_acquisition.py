...
def main() -> int:
    ...
    for year in years:
        ...
        for fn in selected:
            ...
            try:
                did_download, n_bytes = download_file(session, url, dest)
                total_bytes += n_bytes
                if did_download:
                    downloaded.append(str(dest))
                    logger.process(f"Downloaded: {dest.name} ({n_bytes / 1024 / 1024:.2f} MB)")
                else:
                    skipped.append(str(dest))
            except Exception as e:
                failed.append({"url": url, "error": f"{type(e).__name__}: {e}"})
                logger.error(f"Failed: {url} -> {dest} ({type(e).__name__}: {e})")
